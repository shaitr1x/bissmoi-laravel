<x-admin-layout>
    ...
    <form method="POST" action="{{ route('admin.blog.update', $blog) }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" class="w-full border rounded p-2" value="{{ old('title', $blog->title) }}" required>
        </div>
        <div class="mb-4">
            <label class="block font-semibold mb-1">Slug (URL)</label>
            <input type="text" name="slug" class="w-full border rounded p-2" value="{{ old('slug', $blog->slug) }}" required>
        </div>
        <div class="mb-4">
            <label class="block font-semibold mb-1">Contenu</label>
            <textarea name="content" class="w-full border rounded p-2" rows="8" required>{{ old('content', $blog->content) }}</textarea>
        </div>
        <div class="mb-4">
            <label class="inline-flex items-center">
                <input type="checkbox" name="published" value="1" class="form-checkbox" {{ old('published', $blog->published) ? 'checked' : '' }}>
