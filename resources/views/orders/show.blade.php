<x-app-layout>
    <div class="bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <nav class="flex mb-6" aria-label="Breadcrumb">
                            ...
                            @endifer space-x-1 md:space-x-3">
                                ...
                                @foreach($order->orderItems as $item)
                                    <div class="flex items-center space-x-4 p-4 border border-gray-200 rounded-lg">
                                        <div class="flex-shrink-0">
                                            <x-product-image :product="$item->product" size="small" class="w-16 h-16" />
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <h3 class="text-lg font-medium text-gray-900">{{ $item->product->name }}</h3>
                                            <p class="text-sm text-gray-600">Vendeur: {{ $item->product->user->shop_name ?? $item->product->user->name }}</p>
                                    ...
                                    <div class="space-y-2">
                                        @if($order->payment_method === 'cinetpay')
                                            <div class="flex items-center text-sm text-gray-700">
                                                ...
                                                <span>Paiement électronique (CinetPay)</span>
                                            </div>
    ...
    @if($order->payment_method === 'cinetpay')
        @include('orders.partials.cinetpay-payment-modal')
    @endif
</x-app-layout>
